...
|-----------|---------|-------|
| Batch metadata (Flash) | ~$15 | 50% batch discount |
| Contextual prefixes (Flash online) | ~$12 | LLM calls in Phase 3 |
| RAPTOR summaries (Flash online) | ~$5 | Section summaries |
| Embeddings (embedding-2-preview) | ~$2 | Vertex AI REST |
| **Total** | **~$34** | |
...
GEMINI_VERTEXAI_PROJECT=smriti-ingest-a
GEMINI_VERTEXAI_LOCATION=us-central1
GOOGLE_APPLICATION_CREDENTIALS=ingestion/accounts/account_a.json
GCS_BUCKET=smriti-batch-ingestion-a

# Shared DB credentials (same across all workers)
DATABASE_URL=postgresql+asyncpg://user:pass@hostinger-ip:5432/smriti
PINECONE_API_KEY=pk-xxx
PINECONE_HOST=https://smriti-xxx.svc.xxx.pinecone.io
