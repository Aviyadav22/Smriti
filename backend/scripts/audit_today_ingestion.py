...
def check_ocr_title(title):
    ...
    if " 0F " in title or " 0f " in title:
        ...
        issues.append(
            f"title has OCR artifact '0F' instead of 'OF': ...{title[max(0,idx-10):idx+12]}..."
        )
    if re.search(r"[^\x20-\x7E\u0900-\u097F.,&()/\\:;\-\'@#$%^*!?\[\]{{}}|~`+= ]", title):
        garbled = [
            c
            for c in title
            if not re.match(r"[\x20-\x7E\u0900-\u097F.,&()/\\:;\-\'@#$%^*!?\[\]{}|~`+= ]", c)
        ]
        if garbled:
    ...
    for c in cases:
        ...
        if len(name_part) > 5:
            if name_part in seen_names:
                issues.append(
                    f"cases_cited duplicate: '{c[:60]}' vs '{seen_names[name_part][:60]}'"
                )
            else:
                seen_names[name_part] = c
        if re.match(r"^\s*\(\d{4}\)\s+\d+\s+\w+\s+\d+\s*$", c) or re.match(
            r"^\s*\d{4}\s+\(\d+\)\s+\w+\s+\d+\s*$", c
        ):
