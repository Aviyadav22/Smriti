...
VERB_WORDS = {"is", "was", "were", "shall", "should", "hold", "held", "has", "have",
              "had", "may", "can", "will", "would", "could", "must", "being", "been",
              "provide", "provides", "provided", "said", "says", "stated", "states",
              "filed", "contended", "submitted", "argued", "observed", "directed",
              "ordered", "granted", "dismissed", "allowed", "rejected", "upheld"}
...
def has_garbled_nonascii(s):
    ...
    for c in s:
        o = ord(c)
        if 128 <= o < 256 and c not in ('é', 'è', 'ê', 'ë', 'ñ', 'ü', 'ö', 'ä', '§', '°', '–', '—', ''', ''', '"', '"'):
            garbled += 1
    return garbled > 2
...
def main():
    ...
    year_stats = defaultdict(lambda: {
        'count': 0, 'confidence_sum': 0, 'confidence_count': 0,
        'ratio_populated': 0, 'clean_acts': 0, 'clean_cites': 0
    })
    ...
    for row in rows:
        ...
        if r['year'] and r['decision_date']:
            ...
            if abs(r['year'] - dd_year) > 0:
                issues.append(('CRITICAL', 'year != decision_date year',
        ...
        if check_case_type_vs_number(r['case_type'], r['case_number']):
            issues.append(('CRITICAL', 'case_type contradicts case_number',
        ...
        if r['extraction_confidence'] is not None and r['extraction_confidence'] < 0.5:
            issues.append(('CRITICAL', 'extraction_confidence < 0.5',
        ...
        if r['text_length'] is not None and r['text_length'] < 1000:
            issues.append(('CRITICAL', 'text_length < 1000',
                           f"length={r['text_length']}"))
        ...
        if bare_count > 0:
            ...
            bare_cite_cases.append((cid, year, r['title'][:50] if r['title'] else '', bare_count, len(cites)))
        ...
        if not r['ratio_decidendi'] or (isinstance(r['ratio_decidendi'], str) and len(r['ratio_decidendi'].strip()) == 0):
        ...
        if not r['outcome_summary'] or (isinstance(r['outcome_summary'], str) and len(r['outcome_summary'].strip()) == 0):
        ...
        if not r['headnotes'] or (isinstance(r['headnotes'], str) and len(r['headnotes'].strip()) == 0):
        ...
        if check_bench_coram_mismatch(r['bench_type'], r['coram_size']):
            ...
            issues.append(('MEDIUM', 'bench_type/coram_size mismatch',
        ...
        if r['extraction_confidence'] is not None and r['extraction_confidence'] < 0.7:
            low_confidence_cases.append((cid, year, r['title'][:60] if r['title'] else '', r['extraction_confidence']))
