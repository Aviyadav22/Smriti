...
def _is_heading(line: str) -> bool:
    ...
    return bool(
        len(stripped) >= 4 and stripped == stripped.upper() and stripped.replace(" ", "").isalpha()
    )
...
async def export_to_docx(
    ...
    *,
    ...
    doc.core_properties.created = datetime.now(UTC)
    doc.core_properties.comments = f"Generated by Smriti AI — {template.display_name}"
...
def _render_markdown_to_docx(
    doc, content: str, body_size: int = 12, heading_size: int = 14
) -> None:
    ...
    while i < len(lines):
                    ...
                    for c_idx, cell_text in enumerate(row):
                        if c_idx < len(header_cells):
                            ...
                            for sub_line in cell_text.split("<br>"):
                                p = (
                                    cell.paragraphs[0]
                                    if sub_line == cell_text.split("<br>")[0]
        ...
        if stripped[0].isdigit() and ". " in stripped[:5]:
            while (
                i < len(lines)
                and lines[i].strip()
                and lines[i].strip()[0:1].isdigit()
                and ". " in lines[i].strip()[:5]
            ):
                item_text = (
                    lines[i].strip().split(". ", 1)[1]
                    if ". " in lines[i].strip()
                    else lines[i].strip()
                )
