...
LIMITATION_SCHEDULE: Final[dict[str, LimitationPeriod]] = {
    ...
    "cheque_bounce_complaint": LimitationPeriod(0, "Complaint under S.138 NI Act", 0, 1, 0, "Within 1 month of expiry of 15-day notice period"),
    "bail_application": LimitationPeriod(0, "Bail application", 0, 0, 0, "No limitation; can be filed anytime during custody"),
    "anticipatory_bail": LimitationPeriod(0, "Anticipatory bail", 0, 0, 0, "No limitation; file before arrest"),

    # --- Appeals ---
    "civil_appeal_first": LimitationPeriod(116, "First appeal from decree", 0, 0, 90, "90 days from date of decree"),
    "civil_appeal_second": LimitationPeriod(100, "Second appeal (S.100 CPC)", 0, 0, 90, "90 days"),
    "criminal_appeal": LimitationPeriod(114, "Criminal appeal", 0, 0, 30, "30 days from date of sentence"),
    ...
    # --- Writs and Special ---
    "writ_petition": LimitationPeriod(0, "Writ petition", 0, 0, 0, "No strict limitation; delay is a factor"),
    "consumer_complaint": LimitationPeriod(0, "Consumer complaint (CPA 2019)", 2, 0, 0, "2 years from cause of action"),
    "divorce_petition": LimitationPeriod(0, "Divorce petition", 0, 0, 0, "No limitation; S.14 HMA 1-year bar after marriage"),
    "maintenance_application": LimitationPeriod(0, "Maintenance application (S.125 CrPC)", 0, 0, 0, "No limitation"),
    "insolvency_application": LimitationPeriod(0, "IBC S.7/9/10 application", 3, 0, 0, "3 years from date of default"),

    # --- Miscellaneous ---
    "condonation_of_delay": LimitationPeriod(0, "Application under S.5 Limitation Act", 0, 0, 0, "Must show sufficient cause"),
    "legal_notice_govt": LimitationPeriod(0, "S.80 CPC notice to government", 0, 2, 0, "2-month notice before filing suit"),
}
...
def calculate_deadline(
    ...
    if total_days == 0:
        return {
            ...
            "period": f"{period.period_years}y {period.period_months}m {period.period_days}d" if any([period.period_years, period.period_months, period.period_days]) else "No fixed limitation",
            "deadline": None,
            "within_time": True,
            "notes": period.notes,
        }
