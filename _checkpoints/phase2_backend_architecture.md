# Phase 2: Backend Architecture Deep Dive
...
- **Docs**: Swagger UI at `/docs` and ReDoc at `/redoc` -- only enabled when `APP_DEBUG=true`
- **Config**: `pydantic-settings` (`BaseSettings`) loading from `.env` file, case-insensitive, extras ignored

### Lifespan (Startup / Shutdown)

Managed via `@asynccontextmanager async def lifespan(app)`:
...
**Shutdown sequence (all with 10s timeout guards):**
...
Added in reverse order (last added = outermost). Execution order from outermost to innermost:
...
| `RateLimitExceededError` | 429 | `RATE_LIMITED` (with `Retry-After` header) |
...
All external services are created via `@lru_cache` singleton factory functions in `core/dependencies.py`:

| Factory | Returns | Backed by |
|---------|---------|-----------|
...
| `get_checkpointer()` | LangGraph checkpointer | `AsyncPostgresSaver` (prod) or `MemorySaver` (dev) |
| `get_web_search()` | `WebSearchProvider` | `TavilySearchClient` |
| `get_ik_client()` | `ExternalDocProvider` | `IndianKanoonClient` |

Circuit breakers are configured for: Pinecone (5 failures, 30s cooldown), Neo4j (5/60s), Cohere (3/30s).
...
|--------|------|------|------------|---------|
| POST | `/register` | None | 5/min | Register user with consent (DPDP), auto-login, returns JWT pair |
...
**Search query params:** `q` (1-2000 chars), `court`, `year_from`, `year_to`, `case_type`, `bench_type`, `judge`, `act`, `section` (alias for judgment_section), `page`, `page_size` (1-50), `language` (en|hi)

**Search response:** `results[]` (case_id, score, title, citation, court, year, date, case_type, judge, snippet, bench_type, equivalent_citations, treatment_warning), `total_count`, `page`, `page_size`, `query_understanding` (intent, original_query, expanded_query, search_strategy, filters, entities), `facets`
...
| Method | Path | Auth | Rate Limit | Purpose |
|--------|------|------|------------|---------|
| GET | `/health` | Optional | 60/min | Health check. Minimal `{"status": "healthy|degraded|unhealthy"}` for unauthenticated. Full dependency details for authenticated. 503 if PostgreSQL (critical) is down |
...
| Method | Path | Auth | Rate Limit | Purpose |
|--------|------|------|------------|---------|
| GET | `/data-summary` | JWT | 20/min | Summary of all personal data held (Section 11) |
| POST | `/erasure` | JWT | 5/hour | Delete all personal data, deactivate account (Section 12) |
| POST | `/consent-withdraw` | JWT | 10/hour | Withdraw data processing consent (Section 6) |
| GET | `/consent-status` | JWT | 30/min | Current consent records |

### Admin Review (`/api/v1/admin/review`)
...
### Data Quality (`/api/v1/admin/data-quality`)
...
Massive table -- 70+ columns. Core fields:
...
- **Party & Case**: party_counsel (JSONB, GIN jsonb_path_ops), issue_classification (TEXT[]), fact_pattern_tags (TEXT[])
- **Output Quality**: operative_order (TEXT), conditions_imposed (TEXT[]), costs_awarded (JSONB)
- **Other**: page_map (JSONB), enrichment_status (VARCHAR 20, CHECK: flash_only/pro_enriched/failed), source_dataset (VARCHAR 50, indexed), legal_propositions (JSONB), statute_sections_interpreted (JSONB), fact_pattern_summary (TEXT)
...
| ip_address | VARCHAR | Yes | -- | SHA-256 hashed (DPDP) |
...
## 4. Interfaces (Protocol Classes)

All interfaces are `@runtime_checkable` Protocol classes in `backend/app/core/interfaces/`.
...
- **Access token**: 60 minute expiry, contains `sub` (user_id), `role`, `exp`, `iat`, `jti` (unique token ID), `type: "access"`, `iss: "smriti"`, `aud: "smriti-api"`
- **Refresh token**: 7-day expiry, same claims but `type: "refresh"`, `role: "refresh"`, signed with separate `jwt_refresh_secret_key`
...
- **Clock skew**: 30-second leeway on verification
...
1. **admin**: Full access, can manage ingestion, review cases, correct metadata
2. **researcher**: Standard authenticated access (search, chat, agents, documents)
...
- Auth: 5-20/min depending on endpoint
- Search: 30/min
- Chat: 20/min
...
`security/sanitizer.py` provides three functions:

1. **`sanitize_input(text)`**: Strips HTML tags, null bytes, control characters (preserves whitespace), collapses excessive newlines
2. **`sanitize_search_query(query)`**: sanitize_input + removes prompt injection markers + role-switching patterns + collapses whitespace
3. **`detect_prompt_injection(text)`**: Detects 25+ injection markers (e.g., "ignore previous instructions", "DAN mode", chat-ML tokens), role-switching patterns (`system:`, `assistant:`, etc.), suspicious special character density (>15%)

### Encryption

`security/encryption.py` -- AES-256-GCM field-level encryption:

- **Key**: 64-char hex string or base64-encoded 32-byte key (from `ENCRYPTION_KEY` setting)
- **Nonce**: 12 bytes (96 bits), randomly generated per encrypt
- **Output**: base64(nonce + ciphertext + tag)
- **`safe_decrypt(value)`**: Migration-safe -- decrypts if ciphertext, returns as-is if plaintext. If looks like ciphertext but fails, re-raises error

Used for: chat message content, agent message content

### Audit Logging

`security/audit.py` -- Records to `audit_logs` table:

- IP addresses are SHA-256 hashed with salt before storage (DPDP compliance)
...
`backend/app/worker.py` -- Celery with Redis broker:
...
### Audio Digest Generation (`tasks/audio_tasks.py`)
...
### Document Analysis (`tasks/document_tasks.py`)
...
### Custom Exceptions (`security/exceptions.py`)
...
All error responses follow consistent JSON:
...
Caught by global handler in `main.py`, logged with full traceback, captured to Sentry, returns generic 500 with `INTERNAL_ERROR` code. Never exposes stack traces to client.
...
**Production/Staging**: JSON structured logging via `JSONFormatter`:
```json
{
  "severity": "INFO",
  "message": "...",
...
Both formatters run all output through `_redact()` which replaces:
- Email addresses
- API keys / tokens / passwords
- JWT tokens (eyJ... pattern)
- Aadhaar numbers (12-digit)
- PAN numbers (AAAAA9999A)
- Indian mobile numbers (+91/91/0 prefix + 10 digits starting 6-9)

### Request Tracing

`RequestIDMiddleware` sets a `contextvars.ContextVar` per request, which a `logging.Filter` on the root logger injects into every log record as `request_id`. All logs within a request share the same request ID.

### Silenced Loggers

`httpx`, `httpcore`, `urllib3`, `asyncio` set to WARNING level.

### Audit IP Hashing

IP addresses in audit logs are SHA-256 hashed with salt `smriti_audit_ip_v1` before storage -- raw IPs are never persisted (DPDP compliance).
...
|---------|---------|---------|
| app_env | development | Environment (development, production, staging, test) |
| app_debug | false | Enables /docs, /redoc |
| jwt_access_token_expire_minutes | 60 | Access token TTL |
| jwt_refresh_token_expire_days | 7 | Refresh token TTL |
| bcrypt_cost_factor | 12 | Password hashing rounds |
| database_pool_size | 30 | PG connection pool |
| database_max_overflow | 20 | PG pool overflow |
| database_pool_recycle | 1800 | PG connection recycle (30min) |
| gemini_model | gemini-2.5-pro | Primary LLM |
| gemini_flash_model | gemini-2.5-flash | Fast/cheap LLM |
| gemini_embedding_model | gemini-embedding-2-preview | Embedding model |
| gemini_embedding_dimension | 1536 | Vector dimension |
| cohere_rerank_model | rerank-v4.0-pro | Reranker model |
| search_rrf_k | 60 | RRF fusion parameter |
| search_vector_top_k | 20 | Vector search candidates |
| search_fts_top_k | 20 | FTS candidates |
| search_rerank_top_n | 10 | Final reranked results |
| search_cache_ttl | 300 | Search cache (5min) |
| research_max_refinement_rounds | 2 | Agent research iterations |
| chat_max_history | 10 | Chat context window |
| data_retention_days | 365 | General data retention |
| user_upload_retention_days | 7 | Upload auto-cleanup |
| sentry_traces_sample_rate | 0.1 | Sentry performance sampling |

### Production Validations

The config `model_validator` enforces in production:
- JWT secrets and encryption key must be non-empty and >= 32 chars
- CORS must not contain `*`
- Gemini API key must be set (or Vertex AI enabled)
- Pinecone API key and host must be set
- Cohere API key must be set
- Neo4j password must not be default `smriti_dev`
- Database URL must not contain `localhost`
- Storage must not be `local`, TTS must not be `mock`
- Debug must be false

### Database Connection

`db/postgres.py`:
- SSL enabled for production and Supabase connections
- Supabase Supavisor compatibility: `statement_cache_size=0`, anonymous prepared statements
- NullPool when behind external pooler; otherwise configurable pool with pre-ping
- Pool pressure monitoring via SQLAlchemy events (warns when overflow > 50% of pool_size)

`db/redis_client.py`:
- Async singleton with double-checked locking
- Fails fast on connection error (returns None, does not crash app)
